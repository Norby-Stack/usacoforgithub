...
int main()
{
    ...
    sort(cowshifts.begin(), cowshifts.end());
    ...
    for (int i = 0; i < cowshifts.size(); i++) {
        ...
        if (!cur_active_cows.empty()) {
            total_time += current_time - last;
        }
        if (cur_active_cows.size() == 1) {
            int COW = *cur_active_cows.begin();
            each_cow_single_time[COW] += current_time - last;
        }
        if (get<1>(cowpoint)) {
    ...
    cout << total_time - *min_element(each_cow_single_time.begin(), each_cow_single_time.end()) << endl;
}